''' 
    subset class CIFAR100 classes reference code:
...
# hyperparameters : currently ~ 85.0%
num_epochs = 10
batch_size = 4
learning_rate = 0.01
...
#classes 
classLarge = {'bear': 3 , 'skunk': 75  ,'tiger': 88 , 'wolf': 97} #'elephant':31 ,
LargeClasses = ('bear', 'skunk' , 'tiger' , 'wolf')
...
# create custom dataset
train_set1 = DatasetMaker((get_class(train_images , train_labels, classLarge['bear']),
                            get_class(train_images, train_labels, classLarge['skunk']),
                            get_class(train_images, train_labels, classLarge['tiger']),
                            get_class(train_images, train_labels, classLarge['wolf'])), TRANSFORM)
                            
test_set1 = DatasetMaker([get_class(test_images , test_labels , classLarge['bear']),
                            get_class(test_images , test_labels, classLarge['skunk']),
                            get_class(test_images , test_labels, classLarge['tiger']),
                            get_class(test_images, test_labels, classLarge['wolf'])], TRANSFORM_NO_AUG)
...
# Neural net : 5 convolutional layers , 4 pooling layers.
...
for epoch in range(num_epochs):
    ...
    for i, (images, labels) in enumerate(train_loader):
        ...
        optimizer.zero_grad() # empty grads from previous iteration
        loss.backward()
        optimizer.step()
...
PATH = 'detection\\trained_weights\\cnn_animals.pth'
torch.save(model.state_dict(), PATH)


# compute accuracy : test data
with torch.no_grad():
    n_correct = 0
    n_samples = 0
    ...
    for i in range(4):
        ...
        print(f'Accuracy of {LargeClasses[i]}: {acc} %')
